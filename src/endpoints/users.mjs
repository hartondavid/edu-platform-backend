...
router.get('/searchTeacher', userAuthMiddleware, async (req, res) => {
    const { searchField } = req.query;
    ...
    try {
        ...
        const teachers = await (await db.getKnex())('users')
            ...
            // .whereNotIn('users.id', db('class_teachers').select('teacher_id'))
            .select('users.*');


        if (teachers.length === 0) {
            return sendJsonResponse(res, false, 404, 'Nu există profesori!', []);
        }
        ...
        console.error(err);
        return sendJsonResponse(res, false, 500, 'An error occurred while retrieving teachers', null);
    }
})

router.get('/searchTeacherForSubject', userAuthMiddleware, async (req, res) => {
    const { searchField } = req.query;
    ...
    try {
        ...
        const teachers = await (await db.getKnex())('users')
            ...
            // .whereNotIn('users.id', db('subject_teachers').select('teacher_id'))
            .select('users.*');
